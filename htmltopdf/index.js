$(document).ready(function () {
  ...
  async function setDynamicFields() {
    ...
    async function insertSummaryData() {
      ...
      document.title = sanitizeHTML(collectionInfo.name + " Report");
      document.querySelector("#summary-header").innerHTML = sanitizeHTML(collectionInfo.name + " Report");
      ...
      document.querySelector("#totalIterations").innerHTML = sanitizeHTML(runStats.iterations.total.toString());
      document.querySelector("#totalAssertions").innerHTML = sanitizeHTML(runStats.assertions.total.toString());
      document.querySelector("#totalFailedTests").innerHTML = sanitizeHTML((runStats.requests.failed +
        runStats.prerequestScripts.failed +
        runStats.testScripts.failed +
        runStats.assertions.failed).toString());
      document.querySelector("#totalRequestsBadge").innerHTML = sanitizeHTML(runStats.items.total.toString());
      document.querySelector("#totalFailedBadge").innerHTML = sanitizeHTML(runFailures?.length.toString());
  
      document.querySelector("#collectionName").innerHTML = sanitizeHTML(collectionInfo.name);
      document.querySelector("#environmentName").innerHTML = sanitizeHTML(environment.name ?? "Global");
  
      document.querySelector("#totalRunDuration").innerHTML = sanitizeHTML(`${((timings.completed - timings.started) / 1000).toFixed(1)}s`);
      document.querySelector("#averageResponseTime").innerHTML = sanitizeHTML(`${timings.responseAverage.toFixed(0)}ms`);
  
      document.querySelector("#totalRequests").innerHTML = sanitizeHTML(runStats.requests.total.toString());
      document.querySelector("#failedRequests").innerHTML = sanitizeHTML(runStats.requests.failed.toString());
      document.querySelector("#totalTestScripts").innerHTML = sanitizeHTML(runStats.testScripts.total.toString());
      document.querySelector("#failedTestScripts").innerHTML = sanitizeHTML(runStats.testScripts.failed.toString());
      document.querySelector("#totalAssertions2").innerHTML = sanitizeHTML(runStats.assertions.total.toString());
      document.querySelector("#failedAssertions").innerHTML = sanitizeHTML(runStats.assertions.failed.toString());
    ...
    const runIterations = async () => {
      ...
      uniqueIterations.forEach((iteration) => {
        ...
        iterationTab.innerHTML = `<a class="nav-link custom-tab" href="#" onclick="showIteration(${iteration})">${sanitizeHTML((iteration + 1).toString())}</a>`;
        iterationList.appendChild(iterationTab);
      });
      document.getElementById("iterationTitle").textContent = sanitizeHTML(`${uniqueIterations.length} Iteration available to view`);
    ...
    async function showIteration(iteration) {
      ...
      document.getElementById("iterationSelected").innerHTML = sanitizeHTML("Iteration " + (iteration + 1) + " selected");
      ...
      if (items.length && items[0].item) {
        ...
        await processIndividualItems(items, iteration);
      }
    }
  
    async function processItems(items, iteration = 0, parentName = "", parentPath = []) {
      const runResponse = await fetch(`${jsonHostBaseUrl}/run`);
      const runData = await runResponse.json();
      ...
      const baseUrl = envData.values.length ? envData.values.find((value) => value.key === "baseUrl").value : 'http://localhost';
      for (const item of items) {
        ...
        folderHeader.innerHTML = `
          <a data-toggle="expand" href="#" data-target="#folder-expand-${sanitizeHTML(folderId)}" aria-expanded="true" aria-controls="expand" id="folder-${sanitizeHTML(folderId)}" class="expand text-dark z-block">
            <i class="fas fa-info-circle float-left resultsInfoPass"></i>
            ${sanitizeHTML(folderName)} - ${sanitizeHTML(folderRequests.length.toString())} Requests in the folder
            <i class="float-lg-right fa fa-chevron-down" style="padding-top: 5px"></i>
          </a>
        `;
  
        const folderCollapse = document.createElement("div");
        folderCollapse.id = `folder-expand-${sanitizeHTML(folderId)}`;
        folderCollapse.className = "expand";
        folderCollapse.setAttribute("aria-labelledby", `folder-${sanitizeHTML(folderId)}`);
  
        for (let i = 0; i < folderRequests.length; i++) {
          const request = folderRequests[i];
          if (request.item) {
            const nestedRequestsCount = await processItems([request], iteration, folderName, currentPath);
            ...
            const executions = requestIndividual.filter((exec) => exec.item.id === requestId && exec.cursor.iteration === iteration);
            if (executions.length === 0) continue;
  
            const execution = executions[0];
            const requestUrl = `${baseUrl}/${execution.item.request.url.path}`;
            const responseCode = execution.response ? `${execution.response.code} - ${execution.response.status}` : "-";
            const responseTime = execution.response ? `${execution.response.responseTime}ms` : "0ms";
            const responseSize = execution.response ? `${execution.response.responseSize}B` : "0B";
            ...
            requestHeader.innerHTML = `
              <a data-toggle="expand" href="#" data-target="#expand-${sanitizeHTML(requestId)}" aria-expanded="false" aria-controls="expand" id="requests-${sanitizeHTML(requestId)}" class="expanded text-white z-block">
                Iteration: ${sanitizeHTML((iteration + 1).toString())} - ${sanitizeHTML(requestName)}
                <i class="float-lg-right fa fa-chevron-down" style="padding-top: 5px"></i>
              </a>
            `;
  
            const requestCollapse = document.createElement("div");
            requestCollapse.id = `expand-${sanitizeHTML(requestId)}`;
            requestCollapse.className = "expand";
            requestCollapse.setAttribute("aria-labelledby", `requests-${sanitizeHTML(requestId)}`);
                ...
                <div class="col-sm-6 mb-3">
                  <div class="card border-info">
                    <div class="card-body" style="height: 255px">
                      ...
                      <span><i class="fas fa-info-circle"></i></span><strong> Request URL:</strong> <a href="${sanitizeHTML(requestUrl)}" target="_blank">${sanitizeHTML(requestUrl)}</a><br />
                    </div>
                  </div>
                </div>
                <div class="col-sm-6 mb-3">
                  <div class="card border-info">
                    <div class="card-body pb-0">
                      ...
                      <span><i class="fas fa-info-circle"></i></span><strong> Response Code:</strong> <span class="float-right badge-outline badge badge-success">${sanitizeHTML(responseCode)}</span><br />
                      <span><i class="fas fa-stopwatch"></i></span><strong> Mean time per request:</strong> <span class="float-right badge-outline badge badge-success">${sanitizeHTML(responseTime)}</span><br />
                      <span><i class="fas fa-database"></i></span><strong> Mean size per request:</strong> <span class="float-right badge-outline badge badge-success">${sanitizeHTML(responseSize)}</span><br />
                    </div>
                    <div class="card-body pt-0">
                      <hr />
                      <h5 class="card-title text-uppercase text-white text-center bg-info">Test Pass Percentage</h5>
                      <div class="progress" style="height: 40px">
                        <div class="progress-bar ${requestFailCount > 0 ? "bg-danger" : "bg-success"}" style="width: 100%" role="progressbar">
                          <h5 class="text-uppercase text-white text-center" style="padding-top: 5px"><strong>${sanitizeHTML(passPercentage.toFixed(0).toString())}%</strong></h5>
                        </div>
                      </div>
                    </div>
                  </div>
                          ...
                          <tbody>
                            ${execution?.request?.header
                              ?.map(
                                (header) => `
                                  <tr>
                                    <td>${sanitizeHTML(header.key)}</td>
                                    <td class="requestValues">${sanitizeHTML(header.value)}</td>
                                  </tr>
                                `
                              )
                              .join("")}
                            ...
                            ${execution?.response?.header
                              ?.map(
                                (header) => `
                                  <tr>
                                    <td>${sanitizeHTML(header.key)}</td>
                                    <td class="responseValues">${sanitizeHTML(header.value)}</td>
                                  </tr>
                                `
                              )
                              .join("")}
                    ...
                    <div class="card-body">
                      ...
                      ${
                        ...
                        execution?.response?.stream?.data.length > 0
                          ? `<pre class="bg-light p-3"><div class="responseBody"><code>${sanitizeHTML(new TextDecoder().decode(new Uint8Array(execution?.response?.stream?.data)))}</code></div></pre>`
                          : `<div class="progress" style="height: 40px">
                                    ...
                                    execution.assertions
                                      ? execution.assertions
                                          .map(
                                            (assertion) => `
                                              <tr>
                                                <td>${sanitizeHTML(assertion.assertion)}</td>
                                                <td class="${assertion.error ? "text-danger text-center" : "text-success text-center"}">${sanitizeHTML(assertion.skipped ? "0" : assertion.error ? "0" : "1")}</td>
                                                <td class="${assertion.error ? "text-danger text-center" : "text-success text-center"}">${sanitizeHTML(assertion.skipped ? "0" : assertion.error ? "1" : "0")}</td>
                                                <td class="${assertion.skipped ? "text-danger text-center" : "text-success text-center"}">${sanitizeHTML(assertion.skipped ? "1" : "0")}</td>
                                              </tr>
                                            `
                                          )
                                          .join("")
    ...
    async function processIndividualItems(items, iteration = 0, parentName = "", parentPath = []) {
      const runResponse = await fetch(`${jsonHostBaseUrl}/run`);
      const runData = await runResponse.json();
      const envResponse = await fetch(`${jsonHostBaseUrl}/environment`);
      const envData = envResponse.ok ? await envResponse.json() : [];
      ...
      for (const item of items) {
        ...
        folderHeader.innerHTML = `
          <a data-toggle="expand" href="#" data-target="#folder-expand-${sanitizeHTML(folderId)}" aria-expanded="true" aria-controls="expand" id="folder-${sanitizeHTML(folderId)}" class="expand text-dark z-block">
            <i class="fas fa-info-circle float-left resultsInfoPass"></i>
            ${sanitizeHTML(folderName)} - ${sanitizeHTML(items.length.toString())} Requests in the folder
            <i class="float-lg-right fa fa-chevron-down" style="padding-top: 5px"></i>
          </a>
        `;
        ...
        const execution = executions[0];
        const requestUrl = `${baseUrl}/${execution.item.request.url.path.join("/")}`;
        const responseCode = execution.response ? `${execution.response.code} - ${execution.response.status}` : "-";
        const responseTime = execution.response ? `${execution.response.responseTime}ms` : "0ms";
        const responseSize = execution.response ? `${execution.response.responseSize}B` : "0B";
          ...
          <div class="row">
            <div class="col-sm-6 mb-3">
              <div class="card border-info">
                <div class="card-body" style="height: 255px">
                  <h5 class="card-title text-uppercase text-white text-center bg-info">Request Information</h5>
                  <span><i class="fas fa-info-circle"></i></span><strong> Request Method:</strong> <span class="badge-outline-success badge badge-success">${sanitizeHTML(item.request.method)}</span><br />
                  <span><i class="fas fa-info-circle"></i></span><strong> Request URL:</strong> <a href="${sanitizeHTML(requestUrl)}" target="_blank">${sanitizeHTML(requestUrl)}</a><br />
                </div>
              </div>
            </div>
                        ...
                        ${execution?.request?.header
                          ?.map(
                            (header) => `
                              <tr>
                                <td>${sanitizeHTML(header.key)}</td>
                                <td>${sanitizeHTML(header.value)}</td>
                              </tr>
                            `
                          )
                          .join("")}
              ...
              <div class="card border-info">
                <div class="card-body">
                  ...
                  ${
                    execution?.response?.stream &&
                    execution?.response?.stream?.data.length > 0
                      ? `<pre class="bg-light p-3"><code>${sanitizeHTML(new TextDecoder().decode(new Uint8Array(execution?.response?.stream?.data)))}</code></pre>`
                      : `<div class="progress" style="height: 40px">
                          <div class="progress-bar bg-success" style="width: 100%" role="progressbar">
                            <h5 class="text-uppercase text-white text-center" style="padding-top: 5px"><strong>No response body for this request</strong></h5>
                          </div>
          ...
          <div class="card-deck">
            <div class="card border-danger">
              <div class="card-header bg-danger">
                ...
                >
                  Iteration ${sanitizeHTML((failure.cursor.iteration + 1).toString())} - ${sanitizeHTML(failure.error.name)} - ${sanitizeHTML(failure.parent.name)} - ${sanitizeHTML(failure.source.name)}
                  <i
              ...
              >
                <div class="card-body">
                  <h5><strong>Failed Test: </strong>${sanitizeHTML(failure.source.name)}</h5>
                  <hr />
                  <h5
                  ...
                  <div>
                    <pre><div class="failureBody"><code>${sanitizeHTML(failure.error.message)}</code></pre>
                  </div>
                </div>
              </div>
            </div>
      ...
      if (failures.length > 0) {
        ...
        failuresContainer.innerHTML += `
          <div class="alert alert-danger text-uppercase text-center">
            <h4>Showing ${sanitizeHTML(failures.length.toString())} Failures</h4>
          </div>`;
